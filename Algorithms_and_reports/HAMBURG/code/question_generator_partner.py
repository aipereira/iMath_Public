...
#+------------------------+-------------+----------------+------------+
#| student1@haw-hamburg.de| 15          | 1              | 4          |
#| student1@haw-hamburg.de| 31          | 3              | 2          |
#| student1@haw-hamburg.de| 7           | 2              | 5          |
#|         ...            |    ...      |      ...       |    ...     |
#| student2@haw-hamburg.de| 24          | 4              | 1          |
#| student2@haw-hamburg.de| 2           | 1              | 3          |
#|         ...            |    ...      |      ...       |    ...     |
#+------------------------+-------------+----------------+------------+
def loadStudentAnswers(endpoint, sheets, use_pygsheets = False):
    l = []
    index = [3] + list(range(8, 18))
    for s in sheets:
        if use_pygsheets:
            wks = endpoint.worksheet_by_title(s)
            l.append(wks.get_as_df(has_header=False,start="A2",end=(wks.rows,18)).iloc[:,index])
        else:
            l.append(pd.read_excel(endpoint, sheet_name=s).iloc[:,index])
    df_answers = pd.concat(l, ignore_index=True, axis=0)
    # Determine the number of questions and answers
    num_questions = len(df_answers.columns) // 2
    # Generate column names
    col_names = ['Email']
    for i in range(1, num_questions + 1):
        col_names.append(f'Question{i}')
        col_names.append(f'Answer{i}')
    df_answers.columns = col_names
    return df_answers

def generateRatingDataset(df_answers):
    email_col = df_answers['Email']
    question_cols = df_answers.columns[1::2]
    answer_cols = df_answers.columns[2::2]
    new_df = pd.DataFrame({
        'email': email_col.repeat(len(question_cols)),
        'question': df_answers[question_cols].values.ravel(),
        'answer': df_answers[answer_cols].values.ravel()
    })
    return new_df

question_dict = {}

def generate_dataframe(questions = None):
    sheets = ["Data_output", "partner_output_braganca", "partner_output_Ljubljana", "partner_output_Malaga"]
    df_answers = loadStudentAnswers(sh, sheets, use_pygsheets=True)
    df3 = generateRatingDataset(df_answers)
    ...
    if questions:
        df3 = df3[df3['question'].isin(questions)]

    levels = pd.read_csv('./levelIPB_qts_kw.csv', sep=";").sort_values("Question")
    data_input = df.iloc[0:40, [6,9]].astype(int)
    questions_answers = pd.merge(levels, data_input, how="right", left_on='Question', right_on='id_prototype').drop(columns="id_prototype")

    global question_dict
    question_dict = dict(
        zip(questions_answers['Question'], zip(questions_answers['Level IPB'], questions_answers['correct answer'])))
    df3['evaluation'] = df3.apply(lambda row:
                                        1 if row['answer'] == 4
                                        else 0.4 * int(question_dict[row['question']][0]) + 3 if row['answer'] ==
                                                                                                 question_dict[
                                                                                                     row['question']][1]
                                        else 0.4 * int(question_dict[row['question']][0]) + 1, axis=1)
    return df3


df3 = generate_dataframe()
...
def get_questions(output_row, algo):
    ...
    data = Dataset.load_from_df(df3.drop('answer', axis=1), reader)
    alum = output_row[3]
    max_question = df3["question"].max()
    # print(max_question)
    all_questions = list(range(int(max_question) + 1))
    #  print(all_questions)
    # Find the questions that the student answered and get the unanswered
    already_answered = df3.loc[df3['email'] == alum.lower(), 'question']
    ...
    # next line.
    # print(algo)
    svd = SVD() if algo=="Algo1" else KNNBasic()
    # print(svd)
...
def save_on_google_sheets(list1,list2,list3,algo):
    ...
    student = [list1[3]] * 5
    my_dict = {'email': student, 'question': questions, 'answer': answers}
    updated_rows = pd.DataFrame(my_dict)
    updated_rows['evaluation'] = updated_rows.apply(lambda row:
                                  1 if row['answer'] == 4
                                  else 0.4 * int(question_dict[row['question']][0]) + 3 if row['answer'] ==
                                                                                           question_dict[
                                                                                               row['question']][1]
...
def check_validity(text, new_user, n_clicks):
    ...
    if text and re.match(email_regex, text):
        if new_user:
            return [True, True]
        if not df3[df3['email'] == text.lower()].empty:
            return [False, False]
        else:
            return [True,True]
    else:
...
def update_card_body(nextbutton,testButton,start_button,exitButton,submitbutton,trigger,skipbutton,previuosbutton,evalButton,clickedAnswer,optionList,j,index_previous_question,start,name,surname,university,email,seed,list_index_question,backward,list_answers,trigger_2,next_question,count_eq,df_output,df_output_2,df_output_3,trigger_3,questions,algo, invalid, radio):
    ...
    if(start_button==None or exitButton!=None):
        ...
        df_output[3] = email
        if df3[df3['email'] == email.lower()].empty and not radio:
