...
import proto_three_GiuliaIPB_TabelaPorcentagem
import pickle as pk
import RF_functions
...
proto_three_GiuliaIPB_TabelaPorcentagem.Cache_operations.get_instance().load()
...
def algorithm_for_new_question(output_row):
    return proto_three_GiuliaIPB_TabelaPorcentagem.entrypoint(output_row)
...
def save_on_google_sheets(list1,list2,list3):
    ...
    topic = 'prototype'
    sh = gc.open('averages')
    n_sheet = RF_functions.chooseSheet(topic)
